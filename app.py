...
# 原本的五個維度欄位（成分/劑型/劑量/規格/廠商）之外，新增六個「顯示用」欄位：
# 健保代碼(=藥品代碼)、商品名(=藥品英文名稱)、健保價(=支付價)、ATC碼(=ATC代碼)、
# 有效起日(=CSV原始欄名同名，不需改名)、
# 許可證連結(=藥品代碼超連結，欄位內容顯示文字固定為「連結」，點擊可開啟原始網頁)
FIXED_FIELDS = ["成分", "劑型", "劑量", "規格", "廠商", "健保代碼", "商品名", "健保價", "ATC碼", "有效起日", "許可證連結"]
...
# 這六個欄位是「每一列各自對應的顯示用資訊」（健保代碼/商品名/健保價/ATC碼/有效起日/許可證連結），
# 不是像成分/劑型/劑量/規格/廠商那樣代表資料的分組層級，所以即使剛好跟上一列的值相同
# （例如兩家不同廠商的支付價剛好都是 6.7、都是「-」，或有效起日剛好是同一天），也必須照實顯示，
# 不能被 build_nested_rows() 裡「省略重複值」的大綱模式邏輯誤判成同一群組而被留白。
# 「有效起日」的 CSV 原始欄名跟顯示欄名相同，不需要透過 EXTRA_FIELD_SOURCE_MAP 改名，
# 所以這裡另外明著加上，不能只用 EXTRA_FIELD_SOURCE_MAP.keys() 反推。
EXTRA_DISPLAY_FIELDS = set(EXTRA_FIELD_SOURCE_MAP.keys()) | {"有效起日"}
...
QTY_DEFINITION_NOTE = (
    "ℹ️ 「醫令量(數量)」係加總醫療院所申報之計價醫令；"
    "「含包裹支付醫令量(含包裹數量)」係加總醫療院所申報前述計價醫令，"
    "及其他不計價醫令（如：以日劑藥費 MA1~MA4 醫令申報費用）。"
)
...
if df_raw.empty:
    ...
    if not comps_selected:
        ...
        st.caption("僅開放「成分、劑型、劑量、規格、廠商、健保代碼、商品名、健保價、ATC碼、有效起日、許可證連結」十一個欄位可拖曳排列。")
